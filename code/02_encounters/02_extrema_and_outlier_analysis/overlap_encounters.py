...
for idx, orbit in enumerate(hollman_encounters_per_orbit):
    try:
        if orbit[-1] == hollman_encounters_per_orbit[idx + 1][0]:
...
df = pd.DataFrame(overcounted)
# df.to_csv("overlap_encounters.csv", index=False)

hollman_encounters_per_orbit = np.array(hollman_encounters_per_orbit, dtype=object)
print([len(i) for i in hollman_encounters_per_orbit[orbit_number]])

slicing = len(overcounted) // 3

for encounter, orbit in zip(overcounted[::slicing], orbit_number[::slicing]):
    ...
    for ax in ax_mag:
        ax.axvspan(encounter_times[0], encounter_times[-1], alpha=0.5, color='orange', label=f"Encounter\nEncounters per orbit {len(hollman_encounters_per_orbit[orbit])}")
